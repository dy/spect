...
export default function v(source, map=v=>v, unmap=v=>v) {
  ...
  if (arguments.length) {
    ...
    // NOTE: array/object may have symbol.observable, which redefines default deps behavior
      ...
      let vals = source.length ? [] : {}, keys = Object.keys(source), deps, dchannel = c(), teardown = []
      ...
      subscribe(null, null, () => {
        ...
        teardown.length = 0
        for (let key in deps) if (!deps[key][symbol.observable]().observers.length) {
          depsCache.delete(deps[key])
          deps[key][symbol.dispose]()
        }
      })
    }
    // input
